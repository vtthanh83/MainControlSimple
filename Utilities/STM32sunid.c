...
GPIO_TypeDef* GPIO_PORT[LEDn] = {LED1_GPIO_PORT, LED2_GPIO_PORT, LED3_GPIO_PORT, LED4_GPIO_PORT,
								LEDRED_GPIO_PORT, LEDBGND1_GPIO_PORT, LEDBGND2_GPIO_PORT};

const uint16_t GPIO_PIN[LEDn] = {LED1_PIN, LED2_PIN, LED3_PIN, LED4_PIN, LEDRED_PIN, LEDBGND1_PIN, LEDBGND2_PIN};

const uint32_t GPIO_CLK[LEDn] = {LED1_GPIO_CLK, LED2_GPIO_CLK, LED3_GPIO_CLK, LED4_GPIO_CLK,
								LEDRED_GPIO_CLK, LEDBGND1_GPIO_CLK, LEDBGND2_GPIO_CLK};
...
void LEDInit(Led_TypeDef Led)
{
  ...
  //remap pin PD0, PD1
  if(Led == LED1 || Led == LED4 || Led == ALL_LED) GPIO_PinRemapConfig(GPIO_Remap_PD01, ENABLE);
  if(Led == ALL_LED){
	  for(i = 0; i<LEDn; i++){
