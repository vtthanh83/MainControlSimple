#line 1 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
...
#line 7 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
...
#line 8 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
...
#line 9 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
#line 10 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
...
#line 11 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
...
#line 12 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
...
#line 13 "..\\Libraries\\sunid_lib\\thanh\\src\\pn532.c"
...
pn532_error_t pn532GetRegister (uint16_t ui16Reg, uint8_t * ui8Value)
{
	...
	err = pn532SendCommand(cmd,3,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
	if (err != PN532_ERROR_NONE)
	return err;
...
pn532_error_t pn532SetRegister (uint16_t ui16Reg, uint8_t ui8SymbolMask, uint8_t ui8Value)
{
	...
	if(ui8Value!=0)
		...
		cmd[3] = read &	(~ui8SymbolMask);
	err = pn532SendCommand(cmd,4,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532SAMConfigure(void)
{
	...
	if (err == PN532_ERROR_NONE)
	{
		...
		return err;
	pn532SetRegister(0x6302,0x80,1);
	pn532SetRegister(0x6303,0x80,1);
	pn532SetRegister(0x630D, 0x10, 0);
  return PN532_ERROR_NONE;
}
...
pn532_error_t pn532GetFirmware(void)
{
	...
	err = pn532SendCommand(cmd,1,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532CardList(pn532_modulation_t mod)
{
	...
	if (err == PN532_ERROR_NONE)
	{
		...
		card.authenticated = 0;
		if (card.NFCIDLength > 7)
			return PN532_ERROR_APPLEVELERROR;
...
pn532_error_t pn532CardSelect(void)
{
	...
	cmd[1] = 1;  
	err = pn532SendCommand(cmd,2,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532MifareLogin(uint8_t blockAddr, uint8_t keyType, uint8_t * key)
{
	...
	err = pn532SendCommand(cmd,cmdLen,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532MifareWrite16(uint8_t blockAddr, uint8_t verify, uint8_t * buf)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = card.Tg;  				
	cmd[2] = PN532_MIFARE_WRITE16; 	
	cmd[3] = blockAddr;			
	for(i = 0; i<16;i++)
	  cmd[i+4] = buf[i];
	err = pn532SendCommand(cmd,20,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532DesfileSelect(pn532_modulation_t mod)
{
	...
	err = pn532SendCommand(cmd,11,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532DesfireAuthenticate(uint8_t * key, uint8_t keyno)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_AUTHENTICATION;  	
	cmd[3] = keyno;
    ...
    err = pn532SendCommand(cmd,19,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);    
...
pn532_error_t pn532DesfireChangeKeySetting(uint8_t * keySetting)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_CHANGEKEYSETTING;  	
...
pn532_error_t pn532DesfireGetKeySetting(uint8_t * keySetting, uint8_t * maxKey)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;
	cmd[2] = PN532_DESFIRE_GETKEYSETTING;  	
	err = pn532SendCommand(cmd,3,100);
...
pn532_error_t pn532DesfireGetKeyVersion(uint8_t keyNo, uint8_t * keyVersion)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_GETKEYVERSION;  	
	cmd[3] = keyNo;	
	err = pn532SendCommand(cmd,4,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100+500); 
...
pn532_error_t pn532DesfireChangeKey(uint8_t keyNo, uint8_t * oldKey, uint8_t * newKey)
{
  uint8_t ivec[8];
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_CHANGEKEY;  	
	cmd[3] = keyNo;
	memcpy(cmd+4,token,24);	
	err = pn532SendCommand(cmd,28,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);    
...
pn532_error_t pn532CreateApplication(DESFIRE_CreateAID * app)
{
	...
	err = pn532SendCommand(cmd,8,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532DeleteApplication(DESFIRE_CreateAID * app)
{
	...
	err = pn532SendCommand(cmd,6,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532DesfireGetFileSettings(DESFIRE_FileSetting * fileSetting)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_GETFILESETTING;  	
...
pn532_error_t pn532DesfireChangeFileSettings(uint8_t authenticated,DESFIRE_FileSetting * fileSetting)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_CHANGEFILESETTING;  	
	cmd[3] = fileSetting->fileNo;
	if(authenticated == 0)
	{
		...
		err = pn532SendCommand(cmd,7,100);
		delay_ms(1);
		if (err != PN532_ERROR_NONE)
			return err;
	}
	else
	{
		...
		err = pn532SendCommand(cmd,12,100);
		delay_ms(1);
		if (err != PN532_ERROR_NONE)
			return err;
	}
...
pn532_error_t pn532DesfireCreateFile(DESFIRE_FileSetting * fileSetting)
{
	...
	switch(fileSetting->fileType)
	{
		case DESFIRE_STANDARD_FILE:
			...
			err = pn532SendCommand(cmd,10,100);
			break;
		case DESFIRE_BACKUP_FILE:
		...
		case DESFIRE_LINEAR_FILE:
			...
			err = pn532SendCommand(cmd,13,100);
			break;
		case DESFIRE_CYCLIC_FILE:
...
pn532_error_t pn532DesfireDeleteFile(uint8_t fileNo)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_DELETEFILE;  	
	cmd[3] = fileNo;
...
pn532_error_t pn532DesfireWriteData(uint8_t fileNo, uint32_t offset, uint32_t length, uint8_t cs,const uint8_t * data, int32_t * sizesz)
{
    ...
    *sizesz = length;
	...
	err = pn532SendCommand(cmd,10+prepair,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
	...
	if(*sizesz>52)
	{
		while(index < (*sizesz))
		{
			...
			err = pn532SendCommand(cmd,3+prepair,100);
			delay_ms(1);
			if (err != PN532_ERROR_NONE)
				return err;
			err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532DesfireCredit(uint8_t fileNo, int32_t value, uint8_t cs)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = card.Tg;  				
	cmd[2] = PN532_DESFIRE_CREDIT;  	
	cmd[3] = fileNo;
	memcpy(cmd+4,edata,nsize);	
	err = pn532SendCommand(cmd,4+nsize,100);
	delay_ms(1);
	if (err != PN532_ERROR_NONE)
		return err;
	err = pn532ReadResponse(rec,&size,100);
...
pn532_error_t pn532DesfireLimitedCredit(uint8_t fileNo, int32_t value, uint8_t cs)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = card.Tg;  				
	cmd[2] = PN532_DESFIRE_LIMITTEDCREDIT;  	
	cmd[3] = fileNo;
	memcpy(cmd+4,edata,nsize);	
	err = pn532SendCommand(cmd,4+nsize,100);
	delay_ms(1);
...
pn532_error_t pn532DesfireWriteRecord(uint8_t fileNo, uint32_t offset, uint32_t length, uint8_t cs, uint8_t * data, int32_t * sizesz)
{
    ...
    *sizesz = length;
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = card.Tg;  				
	cmd[2] = PN532_DESFIRE_WRITERECORD;  	
	cmd[3] = fileNo;
	cmd[4] = offset & 0xff;
	cmd[5] = (offset>>8) & 0xff;
	cmd[6] = (offset>>16) & 0xff;
...
pn532_error_t pn532DesfireReadRecord(uint8_t fileNo, uint32_t offset, uint32_t length, uint8_t cs,uint8_t * data, int32_t * sizesz)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = 1;  				
	cmd[2] = PN532_DESFIRE_READRECORD;  	
	cmd[3] = fileNo;
	cmd[4] = offset & 0xff;
	cmd[5] = (offset>>8) & 0xff;
	cmd[6] = (offset>>16) & 0xff;
...
pn532_error_t pn532DesfireClearRecord(uint8_t fileNo)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = card.Tg;  				
	cmd[2] = PN532_DESFIRE_CLEARRECORD;  	
	cmd[3] = fileNo;
...
pn532_error_t pn532DesfireCommitTransaction(void)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = card.Tg;  				
	cmd[2] = PN532_DESFIRE_COMMITTRANSACTION;  	
...
pn532_error_t pn532DesfireAbbortTransaction(void)
{
	...
	cmd[0] = PN532_COMMAND_INDATAEXCHANGE;
	cmd[1] = card.Tg;  				
	cmd[2] = PN532_DESFIRE_ABORTTRANSACTION;  	
