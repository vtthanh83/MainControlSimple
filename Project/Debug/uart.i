#line 1 "..\\Libraries\\sunid_lib\\khoi\\src\\uart.c"
...
#line 2 "..\\Libraries\\sunid_lib\\khoi\\src\\uart.c"
#line 3 "..\\Libraries\\sunid_lib\\khoi\\src\\uart.c"
...
#line 4 "..\\Libraries\\sunid_lib\\khoi\\src\\uart.c"
...
#line 5 "..\\Libraries\\sunid_lib\\khoi\\src\\uart.c"





unsigned char UARTRxbuf[4][64];
unsigned char UART_tempBuf [64];
int UART_index = 0;
...
ErrorCode UARTSendData(uint8_t * buffer, int len, int timeout){
	...
	USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)), 0x01);	    
	...
	for(i = 0; i<len; i++){
		USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)), buffer[i]);	    
...
ErrorCode UARTGetData(uint8_t* buf, int* retlen, int timeout){
	...
	*retlen = len;
	UART_bufPopIndex = (UART_bufPopIndex + 1)% 4;
...
void UART_ISR(){ 
	...
	if(USART_GetITStatus(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)), ((uint16_t)0x0525)) != RESET)
  	{	    
		...
		UART_tempBuf[UART_index] = USART_ReceiveData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)));
		
		if(UART_index == 0)
		{
			if(UART_tempBuf[UART_index] == 0x01)
			...
			UART_index ++;
			if(UART_index == 3)	{
			 	UART_BytesToRx = (UART_tempBuf[1] << 8) + UART_tempBuf[2] + 4; 
			}
			if(UART_index ==  UART_BytesToRx){		
				UART_index = 0;
				if((!UART_bufFull)&&(UART_reflection == FALSE)){
					memcpy(UARTRxbuf[UART_bufPushIndex], UART_tempBuf, (UART_BytesToRx));				
					UART_bufPushIndex = (UART_bufPushIndex + 1)% 4;
